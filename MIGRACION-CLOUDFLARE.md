...
> **Estado (2026-08-27):** Fase 0 ✅ · Fase 1 ✅ (motor en `src/rules/`, 64 tests) · Fase 2 ✅ (Worker + `SalaDO`, hibernation/alarms) · Fase 3 ✅ (Better Auth + D1 sembrada, API admin, mazo del DO desde D1) · Fase 4 ✅ (**el cliente ya no usa Supabase**: `salaClient` WS con reconexión/heartbeat/cola, Lobby/OnlineGame por snapshot, Admin contra `/api/admin/cartas`, stub `x-mcm-uid` eliminado, `@supabase/supabase-js` desinstalado; 85 tests + playtest con UI real y 3 bots contra `wrangler dev`, reconexión tras caída del server verificada). `bun run deploy` = build + migraciones + wrangler deploy. Fase 5 ✅ (CI en `.github/workflows/` con deploy auto-omitido hasta configurar secrets; `scripts/bots.mjs` + `scripts/smoke.mjs`). **Decisión: un solo deploy al terminar todas las fases** — el worker desplegado sigue en la versión Fase 3 hasta entonces. **URL decidida: workers.dev ahora, dominio propio al lanzar (ver Fase 6). Siguiente: el gran deploy + corte (Fases 6/7) cuando el usuario dé la señal; Fase 8 (paridad SP) opcional en cualquier momento.**
>
