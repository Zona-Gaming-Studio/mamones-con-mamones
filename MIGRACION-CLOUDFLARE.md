...
> **Estado (2026-08-27):** Fase 0 ✅ · Fase 1 ✅ (motor en `src/rules/`, 64 tests) · Fase 2 ✅ (Worker + `SalaDO`, hibernation/alarms) · Fase 3 ✅ (Better Auth anónimo+email+Google sobre D1 `mamones-db` —creada, migrada y sembrada con las 1047, staging incluido—, sesión → `X-User-Id`, API admin de cartas con rol, mazo del DO desde D1 con fallback JSON; 85 tests). El stub `x-mcm-uid` sigue como fallback hasta la Fase 4. `bun run deploy` = build + migraciones + wrangler deploy. **Siguiente: Fase 4 (cliente)**.
>
