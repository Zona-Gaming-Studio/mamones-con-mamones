...
> **Estado (2026-08-31):** Fase 0 ✅ · Fase 1 ✅ (motor en `src/rules/`, 64 tests) · Fase 2 ✅ (Worker + `SalaDO`, hibernation/alarms) · Fase 3 ✅ (Better Auth + D1 sembrada, API admin, mazo del DO desde D1) · Fase 4 ✅ (**el cliente ya no usa Supabase**: `salaClient` WS con reconexión/heartbeat/cola, Lobby/OnlineGame por snapshot, Admin contra `/api/admin/cartas`, stub `x-mcm-uid` eliminado, `@supabase/supabase-js` desinstalado; 85 tests + playtest con UI real y 3 bots contra `wrangler dev`, reconexión tras caída del server verificada). Fase 5 ✅ (CI en `.github/workflows/`; `scripts/bots.mjs` + `scripts/smoke.mjs`). Fase 6 ✅ (**corte hecho**: ver Deploy abajo). Siguiente: **Fase 7 a partir del 2026-09-07** (fin de la semana de gracia de la PWA: pausar/borrar el proyecto Supabase `hmptndzxaaoghmioeokc` y borrar el proyecto Pages); Fase 8 (paridad SP) opcional en cualquier momento.
>
> **Deploy (automático desde 2026-08-31):** cada push a `master` que toque código corre `deploy.yml` (tests → build → migraciones D1 → `wrangler deploy`) y publica en **`https://mamones-con-mamones.sergebruni.workers.dev`** — esa es la URL de producción (dominio propio al lanzar). Secrets `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` configurados en el repo; primer deploy automático verificado con `bun scripts/smoke.mjs <url>` (SMOKE AUTH OK). El deploy manual `bun run deploy` hace exactamente lo mismo. El viejo prod **Pages** (`…pages.dev`, congelado en `c616d71`, era Supabase) queda **abandonado**: se borra en la Fase 7.
...
1. ✅ Workflows en `.github/workflows/`: `ci.yml` (PRs: tests + build) y `deploy.yml` (push a master: tests como gate → `bun run deploy` = build + migraciones D1 + wrangler deploy). **Mientras no existan los secrets, `deploy.yml` corre solo los tests y omite el deploy con un aviso** — los pushes no quedan en rojo y el deploy se activa solo al configurar `CLOUDFLARE_API_TOKEN` (plantilla "Edit Cloudflare Workers" + D1:Edit) y `CLOUDFLARE_ACCOUNT_ID` (`729284f1d6ac8d2dc4d9a732cf0e3c10`) en Settings → Secrets del repo.
2. ✅ Herramientas de playtest en el repo: `scripts/bots.mjs` (bots que llenan sillas y juegan solos, contra dev/staging/prod) y `scripts/smoke.mjs` (e2e post-deploy: auth + partida + limpieza).
3. ⏳ **Diferido al final por decisión (2026-08-27): un solo deploy cuando estén todas las fases montadas.** Entonces: configurar secrets, `bun run deploy:staging`, playtest real (4+ teléfonos, clásica y amarga, Piensa Rápido, caídas de red a mitad de ronda, juez desconectado, revancha — con `scripts/bots.mjs` para completar sillas). Criterio de corte: una partida completa sin tocar el código y sin estados colgados.

### Fase 6 — Corte (½ sesión + decisión de URL)
**Realidad del deploy (descubierta 2026-08-27):** producción NO es un Worker sino el proyecto **Cloudflare Pages** `mamones-con-mamones` (URL pública `mamones-con-mamones.pages.dev`, sin dominio propio), y su integración GitHub **se rompió con la migración del repo a la org**: el último build es `c616d71` (pre-migración). Pages no puede alojar Durable Objects, así que el Worker es un deployment aparte y el corte incluye mover el tráfico. **URL — decidido (2026-08-28): opción (a) ahora** — `mamones-con-mamones.sergebruni.workers.dev` es la URL del juego y Pages se borra en el corte (las PWA instaladas en pages.dev mueren; se re-anuncia el enlace). **Cuando el juego salga a producción "de verdad", se compra un dominio propio y se apunta al Worker (opción c)** — con Custom Domains del Worker es un cambio de configuración, sin migración.
   Opciones descartadas: (b) front en Pages + Worker como API (dos deployments para siempre); (c) inmediata (no hay dominio aún).
